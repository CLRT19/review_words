...
def getCSVByDay(i, url = "https://xcx.testdaily.cn/td_wxcx/courseWordsTask/12wLu"):
	...
	for item in soup.find_all('li', class_='exampleItem'):
		word = item.find('span', class_='span1').text.strip().split('\n')[0]  # to avoid grabbing the img tag, we split by newline and grab the first part
		pronunciation = item.find_all('span', class_='span2')[0].text.strip()
		transList = item.find_all('span', class_='span2')[1].text.strip().split('\n')
		translation = ""
		for i in transList:
			translation += i + "; "
		defList = item.find('span', class_='textSpan').text.strip() .split('\n')
		familiarity = '2'
		definition = ""
		for i in defList:
			definition += i + "; "
        
		data.append([word, pronunciation, translation, definition, familiarity])

	# write to CSV
	with open('Review_Day'+str(num)+'.csv', 'w', newline='') as csvfile:
		writer = csv.writer(csvfile)
		writer.writerow(['Word', 'Pronunciation', 'Translation', 'Definition', 'Familiarity']) # header
